...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="prose bg-base-100 min-h-screen m-4 border"><a class="flex min-w-fit min-h-fit m-8"${add_attribute("href", data.previous_route, 0)}>${validate_component(Arrow_back_ios_new, "MaterialSymbolsArrowBackIosNew").$$render($$result, {}, {}, {})}</a> <div class="flex flex-col mx-12 mb-12"><h1 class="text-3xl">${escape(profile.name)}</h1> <h1 class="text-xl">@${escape(profile.username)}</h1></div> <div role="tablist" class="tabs tabs-lifted"><button role="tab" class="${"tab " + escape(selected === Tab.posts && "tab-active", true)}">Posts</button> <button role="tab" class="${"tab " + escape(selected === Tab.comments && "tab-active", true)}">Comments</button></div> <div class="flex flex-col mx-12 my-12">${selected === Tab.posts ? `${validate_component(ContentHolder, "ContentHolder").$$render($$result, { items: profile.posts, user, jwt }, {}, {})}` : `${validate_component(CommentsHolder, "CommentsHolder").$$render($$result, { items: profile.comments, user, jwt }, {}, {})}`}</div></div>`;
});
export {
  Page as default
};
